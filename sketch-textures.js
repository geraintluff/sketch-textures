var sketchTexture = function (canvas, options, callback) {
	...
	if (!options.wrap.x) {
		expectedLineLength = Math.min(expectedLineLength, Math.abs(width/diffX));
	}
	if (!options.wrap.y) {
		expectedLineLength = Math.min(expectedLineLength, Math.abs(height/diffY));
	}
	...
	for (var i = 0; i < lineCount; i++) {
		...
		if (!options.wrap.x) {
			forwardLength = Math.min(forwardLength, (width - midX)/diffX);
			backwardLength = Math.min(backwardLength, midX/diffX);
		}
		if (!options.wrap.y) {
			forwardLength = Math.min(forwardLength, (0 - midY)/diffY);
			backwardLength = Math.min(backwardLength, (midY - height)/diffY);
		}
		if (options.taper) {
